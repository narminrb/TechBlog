...
import CustomLeadership from '../../components/common/CustomLeadership';
import CustomPopular from '../../components/common/CustomPopular';
import CustomInstagram from '../../components/common/CustomInstagram';
import CustomDesign from '../../components/common/CustomDesign';
...
const HomePage = () => {
  ...
  const { data: reviewPagination } = ApiDataHook('/reviewpagination');
  const { data: videoData } = ApiDataHook('/videodata');
  const { data: leadershipData } = ApiDataHook('/leadershipdata');
  const { data: leadership2Data } = ApiDataHook('/leadership2data');
  const { data: popularData } = ApiDataHook('/populardata');
  const { data: instagramData } = ApiDataHook('/instagramdata');
...
{/* section1 */}
<div className="top">
  <div className="container mx-auto my-10 p-4 sm:p-6 md:p-10">
    ...
    <div className="mt-28">
      <div className="section-title font-bold text-2xl md:text-[28px] lg:text-[36px] text-center">
        <h2 className="title">Trending Topics</h2>
      </div>
    </div>

    <div
      className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4 mt-10 mb-8"
      ...
      {leadershipData &&
        leadershipData.map((card) => (
          <CustomLeadership
            ...
            desc={card.desc}
            author={card.author}
            url={`/leadershipdata/${card.id}`}
          />
        ))}
    </div>

    <div className=" col-span-12 md:col-span-4">
      <div className=" gray grid grid-cols-1 gap-2">
      <div
  style={{
...
>
  ...
  >
    Popular on Blogar
      ...
      <div className=" gray grid grid-cols-1 gap-2 mt-[46px]">
            <div
  style={{
    borderBottom: '1px solid #ced0d4',
    paddingBottom: '7px',
...
>
  ...
  >
    Stay in Touch
      ...
      <ul class="flex justify-center mb-5 mt-4 space-x-5">
          <li>
...
<div className="grid grid-cols-10 gap-4 mt-4 mb-6">
  {reviewPagination &&
    reviewPagination.map((card) => (
      <div className="col-span-1">
        <CustomStoryPagination
          key={card.id}
          name={card.name}
...
<div className="grid grid-cols-1 md:grid-cols-2 sm:grid-cols-2 gap-2 mb-10">
      {designData &&
        designData.map((card) => (
          <CustomDesign
            key={card.id}
...
{/* -------------------- */}
      <div className=" gray grid grid-cols-1 gap-2 mt-[35px]">
      <div
  style={{
    borderBottom: '1px solid #ced0d4',
    paddingBottom: '5px',
